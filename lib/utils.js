...
export const detectType = (url) => {
  ...
  if (u.startsWith("magnet:") || /\.torrent(\?.*)?$/.test(u)) return "torrent";
...
export const mediaCategory = (type) => {
  ...
  if (["image", "pdf"].includes(type)) return "Photos";
  if (["model3d"].includes(type)) return "3D Models";
  if (["torrent"].includes(type)) return "Torrents";
...
export const typeLabel = {
  ...
  image: "Image", video: "Video", model3d: "3D Model", pdf: "PDF",
  torrent: "Torrent", link: "Link", unknown: "Link",
};
...
export const typeColor = {
  ...
  image: "#8B5CF6", video: "#F59E0B", model3d: "#06B6D4", pdf: "#EF4444",
  torrent: "#94A3B8", link: "#6B7280", unknown: "#6B7280",
};
...
const parseSheetCSV = (csv, tabName) => {
  ...
  const formatIdx = headers.findIndex((h) => h === "format" || h === "vr" || h === "view");
  const explicitTypeIdx = headers.findIndex((h) => h === "type" || h === "kind");
  ...
  return lines
    ...
    .map((line, i) => {
      ...
      const isSupportedUrl = url.startsWith("http") || url.startsWith("magnet:");
      if (!url || !isSupportedUrl) return null;
      ...
      const explicitType = clean(explicitTypeIdx).toLowerCase();
      let type = explicitType === "torrent" ? "torrent" : detectType(url);
