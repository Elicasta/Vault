...
// Types the user can put in the "type" column to override auto-detection
const EXPLICIT_TYPES = new Set([
  "youtube","vimeo","video","audio","music",
  "image","pdf","epub","doc","gdrive","link",
...
export const detectType = (url) => {
  ...
  if (/\.(doc|docx|txt|rtf|pages|md)(\?.*)?$/.test(u)) return "doc";
...
export const mediaCategory = (type) => {
  ...
  if (["pdf","epub","doc"].includes(type)) return "Reading";
  if (["gdrive","gdrive-folder"].includes(type)) return "Drive";
  return "Links";
};

// Filter pill categories shown inside a collection view
export const FILTER_CATS = ["All","Videos","Photos","Music","Reading","Links"];
...
export const typeLabel = {
  ...
  image:"Image", video:"Video", audio:"Audio", music:"Music",
  pdf:"PDF", epub:"eBook", doc:"Doc",
...
export const typeColor = {
  ...
  image:"#8B5CF6", video:"#F59E0B", audio:"#EC4899", music:"#EC4899",
  pdf:"#EF4444", epub:"#10B981", doc:"#3B82F6",
...
const parseSheetCSV = (csv, tabName) => {
  ...
  // Column index resolution — maps user's actual header names to app fields
  ...
  return lines
    ...
    .map((line, i) => {
      ...
      // Explicit type in the sheet overrides URL-based detection
